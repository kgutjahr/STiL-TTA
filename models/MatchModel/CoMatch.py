'''
CoMatch pytorch lightning module
'''
...
from models.MatchModel.comatch_model import CoMatchModel


class CoMatch(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.save_hyperparameters(hparams)

        self.model = CoMatchModel(hparams)
        print('Use CoMatch.py')
        ...
        self.thr = hparams.co_threshold
        self.contrast_th = hparams.contrast_th
        self.lam_c = hparams.lam_c
        self.lam_u = hparams.lam_u

        self.use_pseudo = False
        self.start_epoch = hparams.start_epoch
        
        print(f'CoMatch threshold: {self.thr}, contrastive threshold: {self.contrast_th}, lam_c: {self.lam_c}, lam_u: {self.lam_u}')

        print(f'Model backbone: {self.model.encoder}')
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        outputs_x, outputs_u_s0, lbs_x, probs, Q, sim = self.model((images_x, targets_x),batch_u,epoch=current_epoch)

        loss_x = self.criterion(outputs_x, lbs_x)
        ...
        lam_c = min(current_epoch+1, self.lam_c)
        ...
        with torch.no_grad():
            prob_x = torch.softmax(outputs_x.detach(), dim=1)
            prob_u = torch.softmax(outputs_u_s0.detach(), dim=1)
        pseudo_label = probs.detach()
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        """
        x, y = batch
        y_hat = self.model((x,y), is_eval=True)
