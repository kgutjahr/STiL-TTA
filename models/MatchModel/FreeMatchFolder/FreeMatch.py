'''
FreeMatch pytorch lightning module
'''
...
from models.MatchModel.FreeMatchFolder.freematch_model import FreeMatchModel


class FreeMatch(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.save_hyperparameters(hparams)

        self.model = FreeMatchModel(hparams)
        print('Use FreeMatch.py')
        ...
        self.criterion_consistency = nn.CrossEntropyLoss()

        self.lambda_u = hparams.lambda_u
        self.lambda_e = hparams.lambda_e


        self.use_pseudo = False
        self.start_epoch = hparams.start_epoch
        
        print(f'FreeMatch lambda_u: {self.lambda_u}, lambda_e: {self.lambda_e}')
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        logits_x_lb, pseudo_label, logits_x_ulb_s, mask, ent_loss  = self.model(images_x, images_u_w, images_u_s, labels=targets_x, index=index)

        # supervised loss
        sup_loss = self.criterion_ce(logits_x_lb, targets_x)
        
        unsup_loss = self.criterion_consistency(logits_x_ulb_s, pseudo_label)

        if current_epoch <= self.start_epoch:
            loss = sup_loss
        else:
            loss = sup_loss + self.lambda_u * unsup_loss + self.lambda_e * ent_loss
        ...
        with torch.no_grad():
            prob_x = torch.softmax(logits_x_lb.detach(), dim=1)
            prob_u = torch.softmax(logits_x_ulb_s.detach(), dim=1)
