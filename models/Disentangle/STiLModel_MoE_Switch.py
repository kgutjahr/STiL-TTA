...
from models.Disentangle.utils.STiLModel_backbone_MoE_Switch import DisCoAttentionBackbone
...
class STiLModel_MoE_Switch(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        experts_y_m, x_si_enhance, x_si, x_ai, x_st_enhance, x_st, x_at, x_c, y_MoE, MoE_weights, load_loss = self.model.forward_all(x=[im_views, tab_views], y=y)
        ...
        with torch.no_grad():
            if self.use_ema:
                ...
                experts_y_m_e, x_si_enhance_e, _, _, x_st_enhance_e, _, _, x_c_e, _, _, _ = self.ema.forward_all(x=[im_views, tab_views], y=y)
                feat_m_e = torch.cat((x_si_enhance_e, x_c_e, x_st_enhance_e), dim=1)
                feat_m_e, _, _ = self.project_3features(feat_m_e, None, None)
            else:
                experts_y_m_e = experts_y_m
        #    # case identification. case1: all the same, case2: two the same, case3: else
            prob_MoE = torch.softmax(y_MoE.detach(), dim=1)
            prob_experts = [torch.softmax(expert_y.detach(), dim=-1) for expert_y in experts_y_m_e]
            
            top1_MoE = torch.argmax(prob_MoE, dim=1)
            top1_experts = [torch.argmax(expert_prob, dim=-1) for expert_prob in prob_experts]
            
            #self.acc_classifier_multi(top1_m, y)
            #self.acc_classifier_image(top1_i, y)
            #self.acc_classifier_tabular(top1_t, y)
            
            entropy_MoE = -torch.sum(prob_MoE * torch.log(prob_MoE + 1e-9), dim=1)
            entropy_experts = [-torch.sum(expert_prob * torch.log(expert_prob + 1e-9), dim=-1) for expert_prob in prob_experts]
            ...
            self.log(f'MoE.classifier.entropy', entropy_MoE, on_epoch=True, on_step=False, batch_size=B_l)
            ...
            balanced_acc_experts = [balanced_accuracy_score(y_true=y.cpu(), y_pred=top1_exp.cpu()) for top1_exp in top1_experts]
            ...
            self.log(f'train.balanced_acc', balanced_acc_MoE, on_epoch=True, on_step=False, batch_size=B_l)
            for i, bal_acc_exp in enumerate(balanced_acc_experts):
                self.log(f'train.expert_no{i}.balanced_acc', bal_acc_exp, on_epoch=True, on_step=False, batch_size=B_l)
        ...
        self.log(f"train.load_loss", load_loss, on_epoch=True, on_step=False)
        ...
        loss = self.alpha*loss_ce + self.beta*loss_itc + self.gamma*(loss_clubi + loss_club_i_est + loss_club_t + loss_club_t_est) + self.omega*load_loss
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        x, y = batch
        # use augmented image and tabular views
        experts_y_m, x_si_enhance, x_si, x_ai, x_st_enhance, x_st, x_at, x_c, y_MoE, MoE_weights, load_loss = self.model.forward_all(x=x, y=y)
        feat_m = torch.cat((x_si_enhance, x_c, x_st_enhance), dim=1)
        feat_m, feat_i, feat_t = self.project_3features(feat_m, x_ai, x_at)
        # =============================  itc ======================================
        loss_itc, logits, labels = self.criterion_itc(feat_i, feat_t)
        ...
        self.log(f"val.load_loss", load_loss, on_epoch=True, on_step=False)
        ...
        prob_experts = [torch.softmax(expert_y.detach(), dim=-1) for expert_y in experts_y_m]
        top1_experts = [torch.argmax(expert_prob, dim=-1) for expert_prob in prob_experts]
        
        entropy_experts = [-torch.sum(expert_prob * torch.log(expert_prob + 1e-9), dim=-1) for expert_prob in prob_experts]
        for i, exp_entr in enumerate(entropy_experts):
            self.log(f'val.expert_no{i}.entropy', exp_entr, on_epoch=True, on_step=False)
        ...
        for i, bal_acc_exp in enumerate(balanced_acc_experts):
            self.log(f'val.expert_no{i}.balanced_acc', bal_acc_exp, on_epoch=True, on_step=False)
        ...
        self.log(f'val.balanced_acc', balanced_acc_MoE, on_epoch=True, on_step=False)        

        # task accuracy
        #y_MoE_hat = torch.softmax(y_MoE.detach(), dim=1)
    ...
    def validation_epoch_end(self, _) -> None:
        ...
        self.print(f'Epoch {self.current_epoch}: val.acc: {epoch_acc_val}')
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> None:
        ...
        experts_y_m, _, _, _, _, _, y_MoE, MoE_weights = self.model.forward(x)
        #print(MoE_weights)        

        y_MoE = torch.softmax(y_MoE.detach(), dim=1)
        #y_hat_m = torch.softmax(y_hat_m.detach(), dim=1)
        ...
        for i, exp_entr in enumerate(entropy_experts):
            self.log(f'test.expert_no{i}.entropy', exp_entr, on_epoch=True, on_step=False)
        
        #entropy_m = -torch.sum(y_hat_m * torch.log(y_hat_m + 1e-9), dim=1)
        ...
        top1_MoE = torch.argmax(y_MoE, dim=1)
        ...
        for i, bal_acc_exp in enumerate(balanced_acc_experts):
            self.log(f'test.expert_no{i}.balanced_acc', bal_acc_exp, on_epoch=True, on_step=False)
