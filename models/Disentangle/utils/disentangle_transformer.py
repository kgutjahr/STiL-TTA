'''
Borrow from PIBD
https://github.com/zylbuaa/PIBD/blob/main/models/disentangle_transformer.py
'''
import torch
...
class MIAttention(nn.Module):
    ...
    def forward(self, histology, pathways, global_feature,return_attn = False):
        ...
        _,N_global,_ = global_feature.shape
        ...
        qkv_global = self.qkv(global_feature).reshape(B, N_global, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        attn_global_x = (attn_global @ torch.cat((v_global,v_histology,v_pathways),dim = 2)).transpose(1, 2).reshape(B, N_global, C)
...
if __name__ == '__main__':
    histology = torch.randn(2, 3, 128)
    pathways = torch.randn(2, 6, 128)
    global_feature = torch.randn(2, 9, 128)
    layer = MITransformerLayer(
            dim=128,
            num_heads=4,
            mlp_ratio=1.,
            qkv_bias=True,
            ...
            proj_drop=0.1,
            drop_path=0.1,)
    histology, pathways, global_feature = layer(histology, pathways, global_feature)
    print(histology.shape, pathways.shape, global_feature.shape) # torch.Size([2, 3, 128]) torch.Size([2, 3, 128]) torch.Size([2, 1, 128])