'''
Borrow from https://github.com/somepago/saint
'''
...
class SAINT(nn.Module):
    def __init__(
        ...
        *,
        ...
        categories_offset = torch.tensor([0,1]+list(categories))
        ...
        if self.cont_embeddings == 'MLP':
            ...
            nfeats = self.num_categories + num_continuous+1
        ...
        cat_mask_offset = torch.tensor([0,2]+[2]*self.num_categories)
        ...
        self.mask_embeds_cat = nn.Embedding(self.num_categories*2+2, self.dim)
        ...
        self.pt_mlp = simple_MLP([dim*(self.num_continuous+self.num_categories+1) ,6*dim*(self.num_continuous+self.num_categories+1)//5, dim*(self.num_continuous+self.num_categories+1)//2])
        self.pt_mlp2 = simple_MLP([dim*(self.num_continuous+self.num_categories+1) ,6*dim*(self.num_continuous+self.num_categories+1)//5, dim*(self.num_continuous+self.num_categories+1)//2])
