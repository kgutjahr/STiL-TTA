...
def grab_weak_image_augmentations(img_size: int, target: str, augmentation_speedup: bool = False) -> transforms.Compose:
  if target.lower() == 'dvm':
    if augmentation_speedup:
      transform = A.Compose([
                    A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.2, 1.0)),
    ...
    print('Using dvm transform for weak augmentation')
  else:
    if augmentation_speedup:
      transform = A.Compose([
                    A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.2, 1.0)),
    ...
    print('Using cardiac transform for weak augmentation')
  return transform


def grab_strong_image_augmentations(img_size: int, target: str, augmentation_speedup: bool = False) -> transforms.Compose:
  if target == 'dvm':
    if augmentation_speedup:
      transform = A.Compose([
                    A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.2, 1.0)),
                    ...
                    A.ToGray(p=0.2),
                    A.GaussianBlur(blur_limit=(19,19), sigma_limit=(0.1,2), p=0.5),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
      ])
    else:
      transform =  transforms.Compose([
              ...
              transforms.RandomGrayscale(p=0.2),
              transforms.RandomApply([transforms.GaussianBlur(kernel_size=19, sigma=(0.1, 2))], p=0.5),
              transforms.Lambda(convert_to_float)])
    print('Using dvm transform for strong augmentation')
  else:
    if augmentation_speedup:
      transform = A.Compose([
                    A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.2, 1.0)),
      ...
      transform =  transforms.Compose([
              ...
              transforms.RandomApply([transforms.GaussianBlur(kernel_size=19, sigma=(0.1, 2))], p=0.5),
              transforms.Lambda(convert_to_float)])
    print('Using cardiac transform for strong augmentation')
  return transform
...
def prepend_paths(hparams):
  ...
  for hp in [
    ...
    'labelled_identification_train', 'data_train_eval_imaging_unlabelled',
    'labels_train_eval_imaging_unlabelled', 'data_train_eval_tabular_unlabelled',
    'labels_train_eval_tabular_unlabelled'
    ]:
